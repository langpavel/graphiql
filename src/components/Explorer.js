...
// Patched by Pavel Lang. Original:
// https://github.com/OneGraph/graphiql-explorer/blob/bf334b6a584b9f394bfaede1126dfec7d77d6db4/src/Explorer.js
    ...
    return (
      <div data-arg-name={arg.name} data-arg-type={argType.name}>
        <span
          ...
          <input readOnly type="checkbox" checked={Boolean(argValue)} />
  ...
  _getSelection = (): ?InlineFragmentNode => {
    const selection = this.props.selections.find(
      sel =>
        ...
        sel.typeCondition &&
        this.props.implementingType.name === sel.typeCondition.name.value,
    );
    if (!selection) {
      return null;
    }
    ...
    return (
      <div>
        <span
          ...
          <input readOnly type="checkbox" checked={Boolean(selection)} />
...
class FieldView extends React.PureComponent<FieldViewProps, {}> {
  ...
  _getSelection = (): ?FieldNode => {
    const selection = this.props.selections.find(
      sel => sel.kind === 'Field' && this.props.field.name === sel.name.value,
    );
    if (!selection) {
      return null;
    }
